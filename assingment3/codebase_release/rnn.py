...
class RNN_Model():
    ...
    def add_model_vars(self):
        ...
        with tf.variable_scope('Composition'):
            ### YOUR CODE HERE
            L = tf.get_variable('L', [self.vocab.total_words, self.config.embed_size])
            W1 = tf.get_variable('W1', [2 * self.config.embed_size, self.config.embed_size])
            b1 = tf.get_variable('b1', [1, self.config.embed_size])
            if self.config.l2:
                tf.add_to_collection('L2', self.config.l2 * (tf.nn.l2_loss(W1)))
            ### END YOUR CODE
    ...
    def add_model(self, node):
        ...
        if node.isLeaf:
            ### YOUR CODE HERE
            curr_node_tensor = tf.expand_dims(tf.gather(L, 
                                            self.vocab.encode(node.word)), 0)
            ### END YOUR CODE
        else:
            ...
            curr_node_tensor = tf.nn.relu(
                                tf.matmul(tf.concat(1,[nt[node.left], nt[node.right]]), W1) + b1)
            ### END YOUR CODE
